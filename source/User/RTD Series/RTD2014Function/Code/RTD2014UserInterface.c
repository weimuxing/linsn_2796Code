...
void UserInterfaceUartIntHandler_SERIALPORT(void)
{
    if(GET_UART_RECEIVE_STATUS() == _TRUE)
    {
	 ...
	 IsrReceiveCommand(0, g_pucUartData[0]);
    ...
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT_EDID:
            ...
            if(UserCommonNVRamGetSystemData(_EDID_D0_SELECT) == _DP_EDID_1080P)
            {
				...
				DebugMessageSystem("tEDID_TABLE_DP_1080P",0);
            }
            else if(UserCommonNVRamGetSystemData(_EDID_D0_SELECT) == _DP_EDID_2560_1440)
            {
				...
				DebugMessageSystem("tEDID_TABLE_DP_2K1K",0);
            }
            else if(UserCommonNVRamGetSystemData(_EDID_D0_SELECT) == _DP_EDID_4K2K_30HZ)
            {
				...
				DebugMessageSystem("tEDID_TABLE_DP_4K2K_30",0);
            }
            else if(UserCommonNVRamGetSystemData(_EDID_D0_SELECT) == _DP_EDID_4K2K_60HZ)
            {
				...
				DebugMessageSystem("tEDID_TABLE_DP_4K2K_60",0);
            }
...
EnumAspectRatioType UserInterfaceGetAspectRatioType(EnumDisplayRegion enumDisplayRegion)
{
    switch(GET_OSD_ASPECT_RATIO_TYPE(SysRegionGetPortType(enumDisplayRegion)))
    {
		...
		case _OSD_ASPECT_RATIO_USER:	//加入用户自定义模式 by zyk@20170829
...
// Output Value : Modified Input Timing
//--------------------------------------------------
void UserInterfaceAdjustInputTiming(EnumDisplayRegion enumDisplayRegion)	//输入裁剪
{
	//by zyk 20170913
	#if(_2K_2K_DISPLAY == _ON)
	...
	if(GetInputCapFountion() == _ENABLE)
	{
		#if 1
	    if(GET_MDOMAIN_DISPLAY_MODE() == _DISPLAY_MODE_PBP_LR)
	    {
			DebugMessageSystem("GET_MDOMAIN_DISPLAY_MODE LR",_DISPLAY_MODE_PBP_LR);
			totalV = GET_MDOUSER_OUTPUT_VHEIGHT();
			firstV = GET_MDOUSER_FIRST_VHEIGHT();
			secondV = GET_MDOUSER_SECOND_VHEIGHT();
			inputV = GET_MDOMAIN_INPUT_VHEIGHT();
			
			scaleV = (u32)((firstV + secondV)*inputV/totalV);
			if(scaleV%2 != 0)
				scaleV += 1;

			DebugMessageSystem("GET_MDOUSER_OUTPUT_HEIGHT",totalV);
			DebugMessageSystem("GET_MDOMAIN_INPUT_HEIGHT",inputV);
			DebugMessageSystem("firstV + secondV",firstV + secondV);
			DebugMessageSystem("scaleV",scaleV);
			if(enumDisplayRegion == _DISPLAY_RGN_LEFT)
			{
				SET_MDOMAIN_INPUT_VHEIGHT(scaleV);
				DebugMessageSystem("_DISPLAY_RGN_LEFT",_DISPLAY_RGN_LEFT);
			...
			else if(enumDisplayRegion == _DISPLAY_RGN_RIGHT)
			{
				DebugMessageSystem("_DISPLAY_RGN_RIGHT",_DISPLAY_RGN_RIGHT);
				DebugMessageSystem("GET_MDOMAIN_INPUT_VSTART",GET_MDOMAIN_INPUT_VSTART());
				DebugMessageSystem("GET_MDOMAIN_INPUT_VHEIGHT",GET_MDOMAIN_INPUT_VHEIGHT());
				SET_MDOMAIN_INPUT_VHEIGHT(inputV - scaleV);
				SET_MDOMAIN_INPUT_VSTART(GET_MDOMAIN_INPUT_VSTART() + scaleV);
				DebugMessageSystem("GET_MDOMAIN_INPUT_VHEIGHT",GET_MDOMAIN_INPUT_VHEIGHT());
				DebugMessageSystem("GET_MDOMAIN_INPUT_VSTART",GET_MDOMAIN_INPUT_VSTART());
			}
	    }
		else if(GET_MDOMAIN_DISPLAY_MODE() == _DISPLAY_MODE_PBP_TB)
		{
			DebugMessageSystem("GET_MDOMAIN_DISPLAY_MODE TB",_DISPLAY_MODE_PBP_TB);
			//DebugMessageSystem("GET_MDOMAIN_INPUT_HWIDTH",GET_MDOMAIN_INPUT_HWIDTH());
			//DebugMessageSystem("GET_MDOMAIN_INPUT_HWIDTH",GET_MDOMAIN_INPUT_HWIDTH());
			
			totalH = GET_MDOUSER_OUTPUT_HWIDTH();
			firstH = GET_MDOUSER_FIRST_HWIDTH();
			secondH = GET_MDOUSER_SECOND_HWIDTH();
			inputH = GET_MDOMAIN_INPUT_HWIDTH();
			
			scaleH = (u32)((firstH + secondH)*inputH/totalH);
			if(scaleH%2 != 0)
				scaleH += 1;
			
			DebugMessageSystem("GET_MDOUSER_OUTPUT_HWIDTH",totalH);
			DebugMessageSystem("GET_MDOMAIN_INPUT_HWIDTH",inputH);
			DebugMessageSystem("firstH + secondH",firstH + secondH);
			DebugMessageSystem("scaleH",scaleH);
	        if(enumDisplayRegion == _DISPLAY_RGN_TOP)
	        {
				SET_MDOMAIN_INPUT_HWIDTH(scaleH);
				DebugMessageSystem("_DISPLAY_RGN_TOP",_DISPLAY_RGN_TOP);
				DebugMessageSystem("GET_MDOMAIN_INPUT_HWIDTH",GET_MDOMAIN_INPUT_HWIDTH());
				DebugMessageSystem("GET_MDOMAIN_INPUT_VHEIGHT",GET_MDOMAIN_INPUT_VHEIGHT());
				DebugMessageSystem("GET_MDOMAIN_INPUT_HSTART",GET_MDOMAIN_INPUT_HSTART());
	        }
			else if(enumDisplayRegion == _DISPLAY_RGN_BOTTOM)
	        {
				DebugMessageSystem("_DISPLAY_RGN_BOTTOM",_DISPLAY_RGN_BOTTOM);
				DebugMessageSystem("GET_MDOMAIN_INPUT_HWIDTH",GET_MDOMAIN_INPUT_HWIDTH());
				DebugMessageSystem("GET_MDOMAIN_INPUT_HSTART",GET_MDOMAIN_INPUT_HSTART());
				SET_MDOMAIN_INPUT_HWIDTH(inputH-scaleH);
	            SET_MDOMAIN_INPUT_HSTART(GET_MDOMAIN_INPUT_HSTART() + scaleH);
				DebugMessageSystem("GET_MDOMAIN_INPUT_HWIDTH",GET_MDOMAIN_INPUT_HWIDTH());
				DebugMessageSystem("SET_MDOMAIN_INPUT_HSTART",GET_MDOMAIN_INPUT_HSTART());
	        }
		}
			...
			if(GET_MDOMAIN_DISPLAY_MODE() == _DISPLAY_MODE_PBP_LR)
			{
				SET_MDOMAIN_INPUT_VHEIGHT(GET_MDOMAIN_INPUT_VHEIGHT() >> 1);
				
				if(enumDisplayRegion == _DISPLAY_RGN_RIGHT)
				{
					SET_MDOMAIN_INPUT_VSTART(GET_MDOMAIN_INPUT_VSTART() + GET_MDOMAIN_INPUT_VHEIGHT());
				}
			}
			else if(GET_MDOMAIN_DISPLAY_MODE() == _DISPLAY_MODE_PBP_TB)
			{
				...
				if(enumDisplayRegion == _DISPLAY_RGN_BOTTOM)
				{
					SET_MDOMAIN_INPUT_HSTART(GET_MDOMAIN_INPUT_HSTART() + GET_MDOMAIN_INPUT_HWIDTH());
				}
			}
		#endif
	}
    ...
    //UserDefineAdjustInputTiming();  //add  input settint for test@20161223
}
