        ...
        case _RIGHT_KEY_MESSAGE:  
            ...
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_OTHER_1);
               ...
               if(GET_OSD_LANGUAGE() >= _RUSSIAN)
                {
                    OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_LANGUAGE_1);
                }
                else
                {
                    OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_LANGUAGE_0);
                }
            OsdDispMainMenuOptionSetting(_OPTION_LANGUAGE_TYPE,_UNSELECT,GET_OSD_LANGUAGE());
             OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _OUTSUBSET);
...
void MenuOtherLanguage(void)
{
    switch(GET_KEYMESSAGE())
    {
        case _MENU_KEY_MESSAGE:
           ...
           SET_OSD_STATE(_MENU_OTHER_LANGUAGE_ADJUST);
            if(GET_OSD_LANGUAGE() >= _RUSSIAN)
               ...
               OsdDispMainMenuItemSelection((0), _OSD_PRE_SELECT);
        ...
        case _EXIT_KEY_MESSAGE:
            ...
            OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(), _OUTSUBSET);
            OsdDispClearSelectColor(_DOWN);//
            // OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_1);//2556
            OsdDispMainMenuIconPage(_UP, _ICON_PAGE_MAIN_2);//2796
            OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_OTHER_0);
            break;
...
void MenuOtherLanguageAdjust(void)
{
    switch(GET_KEYMESSAGE())
       ...
       case _RIGHT_KEY_MESSAGE:
          ...
          if(GET_OSD_LANGUAGE() != _KOREA)
                {
                    if(GET_OSD_LANGUAGE() == _JAPAN)
                    {
                        OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_LANGUAGE_1);//第二页
                    }
    
                OsdDispSliderAndNumber(_MENU_OTHER_LANGUAGE_ADJUST, GET_OSD_LANGUAGE());
                OsdDispMainMenuIconPage(_UP, _ICON_PAGE_OTHER_1);//搞到第二页上去
                OsdDispMainMenuOptionSetting(_OPTION_LANGUAGE_TYPE,_SELECT,GET_OSD_LANGUAGE());
            }
        ...
        case _LEFT_KEY_MESSAGE:
                 ...
                 if(GET_OSD_LANGUAGE() != _ENGLISH)
                    ...
                    if(GET_OSD_LANGUAGE() == _RUSSIAN)
                        {
                              OsdDispMainMenuIconPage(_DOWN, _ICON_PAGE_LANGUAGE_0);//第一页
                        }
                   OsdDispSliderAndNumber(_MENU_OTHER_LANGUAGE_ADJUST, GET_OSD_LANGUAGE());
                 OsdDispMainMenuIconPage(_UP, _ICON_PAGE_OTHER_1);//搞到第二页上去
                 OsdDispMainMenuOptionSetting(_OPTION_LANGUAGE_TYPE,_SELECT,GET_OSD_LANGUAGE());
            ...
            if(g_usBackupValue != GET_OSD_LANGUAGE())
                {
                    ...
                    SET_OSD_LANGUAGE(g_usAdjustValue);  
                     ...
                     SET_OSD_STATE(_MENU_OTHER_LANGUAGE);
                       OsdDispMainMenuIconPage(_UP, _ICON_PAGE_OTHER_1);//加入这页进行刷新-2017.2.6
                      OsdDispMainMenuOptionSetting(_OPTION_LANGUAGE_TYPE,_UNSELECT,GET_OSD_LANGUAGE());
                      OsdDispMainMenuCursor(GET_OSD_STATE(), GET_OSD_STATE_PREVIOUS(),_OUTSUBSET);             
                          if(GET_OSD_LANGUAGE() >= _RUSSIAN)
                               {
