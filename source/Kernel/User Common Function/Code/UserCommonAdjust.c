...
void UserCommonAdjustPipSubTimingFormat(EnumDisplayRegion enumDisplayRegion);
#endif
...
#define DVI_INPUT_PORT		_D4_INPUT_PORT
#define HDMI1_4_INPUT_PORT	_D2_INPUT_PORT
#define HDMI2_0_INPUT_PORT	_D1_INPUT_PORT
#define DP_INPUT_PORT		_D0_INPUT_PORT

BYTE  inputsourceport[4] ={DVI_INPUT_PORT,HDMI1_4_INPUT_PORT,HDMI2_0_INPUT_PORT,DP_INPUT_PORT};
...
void UserCommonAdjustDisplayMode(EnumDisplayMode enumDisplayMode)
{
    ...
    EnumSourceSearchPort enumSourceSearchPort0 = inputsourceport[0];//UserInterfaceGetInputPort0();
#if(_MULTI_DISPLAY_MAX >= 0x02)
    EnumSourceSearchPort enumSourceSearchPort1 = inputsourceport[1];//UserInterfaceGetInputPort1();
#if(_4P_DISPLAY_SUPPORT == _ON)
    EnumSourceSearchPort enumSourceSearchPort2 = inputsourceport[2];//UserInterfaceGetInputPort2();
    EnumSourceSearchPort enumSourceSearchPort3 = inputsourceport[3];//UserInterfaceGetInputPort3();
#endif
#endif
    ...
    switch(enumDisplayMode)
    {
        ...
        case _DISPLAY_MODE_4P:
            ...
            SysRegionUpdateInterface(_DISPLAY_RGN_LB, enumSourceSearchPort2);
            SysRegionUpdateInterface(_DISPLAY_RGN_RT, enumSourceSearchPort1);
...
#endif
  DebugMessageSystem("--UserCommonA02--",  UserInterfaceGetAspectRatioType(enumDisplayRegion));
    switch(UserInterfaceGetAspectRatioType(enumDisplayRegion))		//显示模式
    {
        case _ASPECT_RATIO_FULL://满屏
        default:
        ...
        case _ASPECT_RATIO_FIXED://固定(16:9 4:3..)

#if(_ASPECT_ORIGINAL_MODE_SUPPORT == _ON)
            ScalerMDomainAspectOriginMode(_FALSE);
#endif

			DebugMessageSystem("_ASPECT_RATIO_FIXED 16:9",  _ASPECT_RATIO_FIXED);
            UserInterfaceGetAspectRatio(enumDisplayRegion, &usHRatio, &usVRatio);//获取比例参数

            // Calculate H Width
            PDATA_WORD(0) = (DWORD)GET_MDOMAIN_OUTPUT_VHEIGHT() * usHRatio / usVRatio;//计算宽度
            ...
            else if(PDATA_WORD(0) < GET_MDOMAIN_OUTPUT_HWIDTH())
            {
				...
				//by zyk@20710829
				//SET_MDOMAIN_OUTPUT_VSTART((2160-1600)/2);
                //SET_MDOMAIN_OUTPUT_VHEIGHT(1600);
            }

            break;

        case _ASPECT_RATIO_USER://用户自定议  and by zyk@20170829
			...
			DebugMessageSystem("_ASPECT_RATIO_USER",  _ASPECT_RATIO_USER);
			switch(enumDisplayRegion)
			{
				case _DISPLAY_RGN_LEFT:
					DebugMessageSystem("_DISPLAY_RGN_LEFT",  enumDisplayRegion);
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_PBP_LEFT_POS_X());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_PBP_LEFT_SIZE_X());
					DebugMessageSystem("RGN_TOP OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("RGN_TOP OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());

					DebugMessageSystem("GET_MDOMAIN_OUTPUT_VSTART",GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("GET_MDOUSER_PBP_LEFT_POS_Y",GET_MDOUSER_PBP_LEFT_POS_Y());
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_PBP_LEFT_POS_Y());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_PBP_LEFT_SIZE_Y());
					
					DebugMessageSystem("RGN_TOP OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("RGN_TOP OUTPUT_VWIDTH",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_RIGHT:
					DebugMessageSystem("_DISPLAY_RGN_RIGHT",  enumDisplayRegion);
					DebugMessageSystem("GET_MDOMAIN_OUTPUT_HSTART",GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("GET_MDOUSER_PBP_RIGHT_POS_X",GET_MDOUSER_PBP_RIGHT_POS_X());
					DebugMessageSystem("GET_PBP_H_START_ADJ",GET_PBP_H_START_ADJ());
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART()+GET_MDOUSER_PBP_RIGHT_POS_X()-GET_PBP_H_START_ADJ());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_PBP_RIGHT_SIZE_X());
					DebugMessageSystem("RGN_TOP OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("RGN_TOP OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_PBP_RIGHT_POS_Y());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_PBP_RIGHT_SIZE_Y());

					DebugMessageSystem("RGN_TOP OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("RGN_TOP OUTPUT_VWIDTH",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_TOP:
					DebugMessageSystem("_DISPLAY_RGN_TOP",  enumDisplayRegion);
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_PBP_TOP_POS_X());
					if((GET_MDOUSER_PBP_TOP_SIZE_X()) >= 3840)
						SET_MDOMAIN_OUTPUT_HWIDTH(3840);
					else
						SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_PBP_TOP_SIZE_X());
					DebugMessageSystem("RGN_TOP OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("RGN_TOP OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_PBP_TOP_POS_Y());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_PBP_TOP_SIZE_Y());
				...
				case _DISPLAY_RGN_BOTTOM:
					DebugMessageSystem("_DISPLAY_RGN_BOTTOM",  enumDisplayRegion);
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_PBP_BOTTOM_POS_X());
					if((GET_MDOUSER_PBP_BOTTOM_SIZE_X()) >= 3840)
						SET_MDOMAIN_OUTPUT_HWIDTH(3840);
					else
						SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_PBP_BOTTOM_SIZE_X());

					DebugMessageSystem("RGN_BOTTOM OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("RGN_BOTTOM OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());

					DebugMessageSystem("GET_MDOMAIN_OUTPUT_VSTART",GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("GET_MDOUSER_PBP_BOTTOM_POS_Y",GET_MDOUSER_PBP_BOTTOM_POS_Y());
					DebugMessageSystem("GET_PBP_V_START_ADJ",GET_PBP_V_START_ADJ());
					
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART()+GET_MDOUSER_PBP_BOTTOM_POS_Y()-GET_PBP_V_START_ADJ());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_PBP_BOTTOM_SIZE_Y());

					DebugMessageSystem("RGN_BOTTOM OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("RGN_BOTTOM OUTPUT_VWIDTH",  GET_MDOMAIN_OUTPUT_VHEIGHT());
					
				break;
				case _DISPLAY_RGN_MAIN:
					DebugMessageSystem("_DISPLAY_RGN_MAIN",  enumDisplayRegion);
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART()+GET_MDOUSER_PIP_MAIN_POS_X());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_PIP_MAIN_SIZE_X());
					DebugMessageSystem("RGN_MAIN OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("RGN_MAIN OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART()+GET_MDOUSER_PIP_MAIN_POS_Y());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_PIP_MAIN_SIZE_Y());
					DebugMessageSystem("RGN_MAIN OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("RGN_MAIN OUTPUT_VWIDTH",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_SUB:
					DebugMessageSystem("_DISPLAY_RGN_SUB",  enumDisplayRegion);
					SET_MDOMAIN_OUTPUT_HSTART(48 + GET_MDOUSER_PIP_SUB_POS_X());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_PIP_SUB_SIZE_X());
					DebugMessageSystem("RGN_SUB OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("RGN_SUB OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());

					SET_MDOMAIN_OUTPUT_VSTART(6 + GET_MDOUSER_PIP_SUB_POS_Y());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_PIP_SUB_SIZE_Y());
					DebugMessageSystem("RGN_SUB OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("RGN_SUB OUTPUT_VWIDTH",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_LT:
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_FIRST_REGION_HSTART());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_FIRST_HWIDTH());
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_FIRST_REGION_VSTART());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_FIRST_VHEIGHT());
					DebugMessageSystem("_DISPLAY_RGN_LT OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("_DISPLAY_RGN_LT OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					DebugMessageSystem("_DISPLAY_RGN_LT OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("_DISPLAY_RGN_LT OUTPUT_VHEIGHT", GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_LB:
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_THIRD_REGION_HSTART());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_THIRD_HWIDTH());
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_THIRD_REGION_VSTART());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_THIRD_VHEIGHT());
					DebugMessageSystem("_DISPLAY_RGN_LB OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("_DISPLAY_RGN_LB OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					DebugMessageSystem("_DISPLAY_RGN_LB OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("_DISPLAY_RGN_LB OUTPUT_VHEIGHT",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_RT:
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_SECOND_REGION_HSTART());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_SECOND_HWIDTH());
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_SECOND_REGION_VSTART());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_SECOND_VHEIGHT());
					DebugMessageSystem("_DISPLAY_RGN_RT OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("_DISPLAY_RGN_RT OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					DebugMessageSystem("_DISPLAY_RGN_RT OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("_DISPLAY_RGN_RT OUTPUT_VHEIGHT",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				case _DISPLAY_RGN_RB:
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART() + GET_MDOUSER_FOURTH_REGION_HSTART());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_FOURTH_HWIDTH());
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + GET_MDOUSER_FOURTH_REGION_VSTART());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_FOURTH_VHEIGHT());
					DebugMessageSystem("_DISPLAY_RGN_RB OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("_DISPLAY_RGN_RB OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					DebugMessageSystem("_DISPLAY_RGN_RB OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("_DISPLAY_RGN_RB OUTPUT_VHEIGHT",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				break;
				default:
					DebugMessageSystem("default",  enumDisplayRegion);	
					SET_MDOMAIN_OUTPUT_HSTART(GET_MDOMAIN_OUTPUT_HSTART()+GET_MDOUSER_FIRST_REGION_HSTART());
					SET_MDOMAIN_OUTPUT_HWIDTH(GET_MDOUSER_OUTPUT_HWIDTH());

					DebugMessageSystem("MODE_1P OUTPUT_HSTART",  GET_MDOMAIN_OUTPUT_HSTART());
					DebugMessageSystem("MODE_1P OUTPUT_HWIDTH",  GET_MDOMAIN_OUTPUT_HWIDTH());
					
					SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART()+GET_MDOUSER_FIRST_REGION_VSTART());
					SET_MDOMAIN_OUTPUT_VHEIGHT(GET_MDOUSER_OUTPUT_VHEIGHT());

					DebugMessageSystem("MODE_1P OUTPUT_VSTART",  GET_MDOMAIN_OUTPUT_VSTART());
					DebugMessageSystem("MODE_1P OUTPUT_VHEIGHT",  GET_MDOMAIN_OUTPUT_VHEIGHT());
        ...
        case _ASPECT_RATIO_ORIGIN:
			...
			DebugMessageSystem("_ASPECT_RATIO_ORIGIN 1:1",  _ASPECT_RATIO_ORIGIN);
			DebugMessageSystem("Output Width",  GET_MDOMAIN_OUTPUT_HWIDTH());
			DebugMessageSystem("Input Width",  GET_MDOMAIN_INPUT_HWIDTH());
			DebugMessageSystem("Output height",  GET_MDOMAIN_OUTPUT_VHEIGHT());
			DebugMessageSystem("Input height",  GET_MDOMAIN_INPUT_VHEIGHT());
            // Consider Panel Height and Input Height
            if(GET_MDOMAIN_OUTPUT_VHEIGHT() > GET_MDOMAIN_INPUT_VHEIGHT())//如果输出高度大于输入高度
            {
				
                // Double V height for interlaced timing
                if((GET_MDOMAIN_INPUT_INTERLACE_FLG() == _TRUE) &&
                   (GET_MDOMAIN_INPUT_HWIDTH() > (GET_MDOMAIN_INPUT_VHEIGHT() << 1)))//如果输入宽度大于2倍输入高度
                {
                    // Check if output V height > (input V height x 2)
                    if(GET_MDOMAIN_OUTPUT_VHEIGHT() > (GET_MDOMAIN_INPUT_VHEIGHT() << 1))//如果输出高度大于2倍输入高度
                    {
                ...
                else
                {
					...
					DebugMessageSystem("V height",  GET_MDOMAIN_INPUT_VHEIGHT());
                }

                PDATA_WORD(0) &= 0xFFFC; // V start should be even
                SET_MDOMAIN_OUTPUT_VSTART(GET_MDOMAIN_OUTPUT_VSTART() + (PDATA_WORD(0) / 2));

				DebugMessageSystem("Output Height > Input Height",  GET_MDOMAIN_OUTPUT_VHEIGHT());
				DebugMessageSystem("Output Height > Input Height",  GET_MDOMAIN_INPUT_VHEIGHT());
				DebugMessageSystem("V start",  (PDATA_WORD(0) / 2));
            }
            else
            {
				DebugMessageSystem("Output Height <= Input Height",  (PDATA_WORD(0) / 2));
                usHRatio = 0x01;
            }
            ...
            if(GET_MDOMAIN_OUTPUT_HWIDTH() > GET_MDOMAIN_INPUT_HWIDTH())
            {
				...
				DebugMessageSystem("Output Width > Input Width",  GET_MDOMAIN_OUTPUT_HWIDTH());
				DebugMessageSystem("Output Width > Input Width",  GET_MDOMAIN_INPUT_HWIDTH());
				DebugMessageSystem("H Width",  GET_MDOMAIN_INPUT_HWIDTH());
				DebugMessageSystem("H Start",  (PDATA_WORD(0) / 2));
            }
            else
            {
				DebugMessageSystem("Output Width <= Input Width",  (PDATA_WORD(0) / 2));
                usVRatio = 0x01;
            }

            if((usHRatio != 1) || (usVRatio != 1))
            {
				DebugMessageSystem("usHRatio",  usHRatio);
				DebugMessageSystem("usVRatio",  usVRatio);
				...
				DebugMessageSystem("V Start",  PDATA_WORD(1));
				DebugMessageSystem("V height",  GET_MDOMAIN_OUTPUT_VHEIGHT() + PDATA_WORD(1) * 2);
				...
				DebugMessageSystem("H Start",  PDATA_WORD(1));
				DebugMessageSystem("H Width",  GET_MDOMAIN_OUTPUT_HWIDTH() + PDATA_WORD(1) * 2);
            }
...
void UserCommonAdjustPipSubTimingFormat(EnumDisplayRegion enumDisplayRegion)
{
       ...
       (SysRegionGetDisplayState(enumDisplayRegion) == _DISPLAY_STATE_DISPLAY_READY))
    {
	        ...
	        UserCommonAdjustAspectRatio(enumDisplayRegion);	//计算PIP 子窗口有效区域大小
	#endif
