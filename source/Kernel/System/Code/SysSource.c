...
void SysSourceHandler(void)
{
    ...
    {
		DebugMessageSystem("_POWER_STATUS_NORMAL",GET_POWER_STATUS());
        return;
    }
#endif
      ...
      (_SOURCE_AUTO_SEARCH_STYLE == _SOURCE_POLLING_ACER_STYLE))
        ...
        case _SOURCE_SWITCH_DELL_HOTKEY_AUTO:
            ...
            do
            {
                ...
                ucCount++;
				DebugMessageSystem("ucCount = ",ucCount);
				DebugMessageSystem("g_ucSearchIndex = ",g_ucSearchIndex);
            }
            while(ucCount < _INPUT_PORT_VALID);
...
void SysSourceScanPortInitial(void)
{
    ...
    {
        case _A0_INPUT_PORT:
			DebugMessageSystem("_A0_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
			//DebugMessageSystem("GET_INPUT_PORT",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
#if(_VGA_SUPPORT == _ON)
        ...
        case _D0_INPUT_PORT:
			DebugMessageSystem("_D0_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
        ...
        case _D1_INPUT_PORT:
			DebugMessageSystem("_D1_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
        ...
        case _D2_INPUT_PORT:
			DebugMessageSystem("_D2_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
        ...
        case _D3_INPUT_PORT:
			DebugMessageSystem("_D3_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
        ...
        case _D4_INPUT_PORT:
			DebugMessageSystem("_D4_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
        ...
        case _D5_INPUT_PORT:
			DebugMessageSystem("_D5_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
        ...
        case _D6_INPUT_PORT:
			DebugMessageSystem("_D6_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
#if(_D6_INPUT_PORT_TYPE == _D6_DP_PORT)
        ...
        case _D7_INPUT_PORT:
			DebugMessageSystem("_D7_INPUT_PORT", 1);
			DebugMessageSystem("g_ucSearchIndex",g_ucSearchIndex);
            switch(GET_PORT_TYPE(GET_INPUTPORT_TYPE(g_ucSearchIndex)))
            {
#if(_D7_INPUT_PORT_TYPE == _D7_DP_PORT)
...
{
    BYTE ucTemp = 0;
	//DebugMessageSystem("_INPUT_PORT_VALID = ",_INPUT_PORT_VALID);
    while(ucTemp < _INPUT_PORT_VALID)
    {
		//DebugMessageSystem("ucInputPort = ",ucInputPort);
        if(GET_INPUT_PORT(GET_INPUTPORT_TYPE(ucTemp)) == ucInputPort)
        {
			//DebugMessageSystem("ucTemp = ",ucTemp);
            return ucTemp;
        }

        ucTemp++;
    }
	//DebugMessageSystem("ucTemp = ",0);
    return 0;
}
...
extern void ScalerMeasureInitialSetting(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort *penumSourceSearchPort);
extern void ScalerMeasureInterlaceInitial(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort *penumSourceSearchPort);

void SetInputTimming(EnumSourceSearchPort penumSourceSearchPort)
{

	ScalerMeasureInterfaceInitial(GET_RGN_SOURCE_TYPE(GET_SOURCE_MEASURE_TARGET_INDEX()), &penumSourceSearchPort);

    // Initial measure & auto-function
    ScalerMeasureInitialSetting(GET_RGN_SOURCE_TYPE(GET_SOURCE_MEASURE_TARGET_INDEX()), &penumSourceSearchPort);

    // Initial Interlace Check Block
    ScalerMeasureInterlaceInitial(GET_RGN_SOURCE_TYPE(GET_SOURCE_MEASURE_TARGET_INDEX()), &penumSourceSearchPort);
	
	ScalerTimerCancelTimerEvent(_SYSTEM_TIMER_EVENT_SOURCE_NOSIGNAL_STATE);

	//DebugMessageSystem("Input Timing HP", 			GET_INPUT_TIMING_H_POLARITY());

	SET_INPUT_TIMING_H_POLARITY(1);
	SET_INPUT_TIMING_V_POLARITY(1);
	SET_INPUT_TIMING_INTERLACE_FLG(0);
	SET_INPUT_TIMING_HFREQ(675);
	SET_INPUT_TIMING_HTOTAL(2200);
	SET_INPUT_TIMING_HSTART(270);
	SET_INPUT_TIMING_HWIDTH(1920);
	SET_INPUT_TIMING_HSYNCWIDTH(43);
	SET_INPUT_TIMING_VFREQ(600);
	SET_INPUT_TIMING_VTOTAL(1125);
	SET_INPUT_TIMING_VSTART(34);
	SET_INPUT_TIMING_VHEIGHT(1080);
	SET_INPUT_TIMING_VSYNCWIDTH(3);
	...
	if(UserCommonInterfaceCheckNoSupport(GET_RGN_DISPLAY_REGION(GET_SOURCE_MEASURE_TARGET_INDEX())) == _TRUE)
	{
		...
		ScalerTimerActiveTimerEvent(SEC(0.1), _SYSTEM_TIMER_EVENT_SOURCE_NOSUPPORT_STATE);
	}
}
