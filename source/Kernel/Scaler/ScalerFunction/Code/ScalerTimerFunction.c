...
bit isSysTickTimeout(WORD startTiming, WORD delayTimming);
#endif
...
{
 	u32 endTiming;
	endTiming = ScalerTimerGetTimerCounter();
	//DebugMessageSystem("------endTiming---", endTiming);
	if(endTiming >= startTiming)
	{
		...
		{
			return _TRUE;  // time out
		}
		
	}
	else
	{
		if((endTiming+(WORD_MAX-startTiming)) < delayTimming)
		{
			return _FALSE;
		}
		else
